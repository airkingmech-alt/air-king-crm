...
OpenAI API key setup: create a key in the OpenAI Platform project and save it as `OPENAI_API_KEY` in Render → Air King CRM → Environment, then redeploy. Do not put the key in chat, GitHub, or a VITE_ variable. A ChatGPT login is not a substitute for an API key. No live generation was tested without the owner’s key.

OpenAI provider-switch validation: five focused tests, TypeScript checking, and production build passed. Provider calls were mocked; live generation still requires the owner’s key.
