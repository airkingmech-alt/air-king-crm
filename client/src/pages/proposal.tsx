...
import { addOnServices, formatEquipmentDescription, pricebook, getTieredEquipment } from "@/data/pricebook";
...
export default function Proposal() {
  ...
  const { createWorkOrder, updateQuoteStatus, updateWorkOrder, createInvoice, quotes } = useData();
  ...
  if (!quote) {
    return (
      <div className="p-6">
        ...
        <Link href="/quotes">
          <Button variant="outline" size="sm" className="mt-4">Back to Quotes</Button>
        </Link>
      </div>
    );
  }
  ...
  const handleDownloadPdf = () => {
    toast({
      title: "Preparing PDF",
      description: "Use your browser's print dialog to save this proposal as a PDF.",
    });
    window.print();
  };
  ...
  const addOnTotal = addOnServices
    .filter((a: typeof addOnServices[number]) => selectedAddOns.includes(a.id))
    .reduce((sum: number, a: typeof addOnServices[number]) => sum + a.price, 0);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 proposal-printable">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3 no-print">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowSendDialog(true)} data-testid="button-send-proposal">
          ...
          <Card className="mb-6">
            <CardContent className="p-5">
              ...
              <div className="space-y-2">
                {(selectedTier
                  ? getTieredEquipment(quote.equipmentItems, selectedTier as "Good" | "Better" | "Best")
                  : quote.equipmentItems.map((id) => pricebook.find((p) => p.id === id)).filter(Boolean)
                ).map((item) => {
                  if (!item) return null;
                  return (
                    <div key={item.id} className="flex items-center gap-3 p-2 rounded-lg border border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium">{formatEquipmentDescription(item)}</p>
              ...
              >
                ...
                <CardContent className="p-5">
                  <div className="text-center mb-4">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{option.tier}</p>
                    <p className="text-lg font-bold mt-1">{option.label}</p>
                  </div>

                  <div className="text-center mb-4">
                    <p className="text-2xl font-extrabold tracking-tight">{fmtCurrency(option.customerPrice)}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Installed · all-inclusive</p>
                  </div>

                  <div className="space-y-2 mb-4">
                    <p className="text-xs font-semibold text-muted-foreground mb-1">{option.equipment}</p>
                    <p className="text-xs font-medium text-primary">{option.efficiency}</p>
        ...
        <Card className="mb-6">
          <CardContent className="p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">Optional add-ons — select to add to your package</p>
            ...
            <CardContent className="p-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{fmtCurrency(selectedOption?.customerPrice || 0)}</span>
                </div>
                {selectedAddOns.map((addonId) => {
                  ...
                  return (
                    <div key={addonId} className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">{fmtCurrency(addon?.price || 0)}</span>
                    </div>
                  );
                })}
                <Separator className="my-2" />
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">Total Installed Price</span>
                  <span className="text-xl font-extrabold">{fmtCurrency(grandTotal)}</span>
                </div>
                <div className="flex items-center gap-2 p-2 rounded-lg bg-sky-50 dark:bg-sky-950/20 text-xs text-sky-700 dark:text-sky-400">
                  <TrendingUp size={14} />
                  Est. {fmtCurrency(monthlyEstimate)}/mo with financing · Subject to credit approval · Wisetack
                </div>
              </div>
                ...
                {accepted ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-center gap-2 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900">
                      ...
                      <div>
                        <p className="text-sm font-bold text-emerald-700 dark:text-emerald-400">Proposal Accepted</p>
                        <p className="text-[10px] text-emerald-600 dark:text-emerald-500">Work order created and added to the schedule queue.</p>
                      </div>
                    </div>
                    <Button
                      className="w-full"
                      ...
                      onClick={() => {
                        const inv = createInvoice({
                          ...
                          amount: grandTotal,
                          description: `${quote.title} — ${selectedOption?.tier || ""} package`,
                          workOrderId: undefined,
                        });
                        toast({
                          title: "Invoice created",
                          description: `${inv.id} created for ${quote.customerName} — ${fmtCurrency(grandTotal)}.`,
                        });
                      }}
                  ...
                  <>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        updateWorkOrder(wo.id, { status: "Unscheduled", scheduledDate: undefined, scheduledTime: undefined, technician: undefined });
                    ...
                    <p className="text-[10px] text-center text-muted-foreground">
                      By approving, you authorize Air King to proceed with the selected package. No deposit required.
                    </p>
                  </>
                )}
              </div>
          ...
          <CardContent className="p-5 space-y-3">
            ...
            <div className="space-y-2 text-xs text-muted-foreground">
              <div className="flex items-start gap-2">
                ...
                <span>10-year parts and labor warranty on all Champion equipment</span>
              </div>
              <div className="flex items-start gap-2">
                <Check size={12} className="text-emerald-500 mt-0.5 shrink-0" />
                <span>100% satisfaction guarantee — if you're not happy, we'll make it right</span>
              </div>
              <div className="flex items-start gap-2">
                <Check size={12} className="text-emerald-500 mt-0.5 shrink-0" />
            ...
            <div className="text-[10px] text-muted-foreground">
              ...
              <p>Proposal valid for 30 days. Installation typically scheduled within 1-2 weeks of approval. Balance due upon completion. Financing available through Wisetack (subject to credit approval). Prices include all equipment, labor, and standard materials. Additional charges may apply for non-standard ductwork, electrical upgrades, or permits.</p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="text-center py-4">
          <p className="text-xs text-muted-foreground">
            Air King Mechanical Services LLC · Kansas City, MO · Lic. #HVAC-2019-0442
          </p>
          <p className="text-[10px] text-muted-foreground mt-1">
            Prices subject to change without notice. © 2026 Air King Mechanical Services.
          </p>
        </div>
      </div>
