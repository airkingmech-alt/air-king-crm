...
type Lead={id:string;status:LeadStatus;source:string;name:string;email:string|null;phone:string|null;address:string|null;city:string|null;state:string|null;postal_code:string|null;service_type:string|null;message:string;customer_id:string|null;created_at:string;updated_at:string;company_id:string};
...
export default function Leads(){
 ...
 const linkedLead=new URLSearchParams(window.location.hash.split("?")[1] || "").get("lead");
 useEffect(()=>{if(!isLoading && linkedLead)document.getElementById(`lead-${linkedLead}`)?.scrollIntoView({block:"center"});},[isLoading,linkedLead]);
 ...
 return <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-4">
  ...
  {isLoading?<p className="text-sm text-muted-foreground">Loading leads…</p>:<div className="space-y-3">{shown.map(l=><Card id={`lead-${l.id}`} key={l.id} className={linkedLead===l.id?"ring-2 ring-primary":""}><CardContent className="p-4"><div className="flex flex-col lg:flex-row lg:items-center gap-3"><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><h3 className="font-semibold">{l.name}</h3><Badge className="capitalize" variant={l.status==="new"?"default":"secondary"}>{l.status}</Badge><Badge variant="outline">{l.source.replaceAll("_"," ")}</Badge></div><p className="text-sm text-muted-foreground mt-1">{[l.phone,l.email,l.service_type].filter(Boolean).join(" · ")}</p>{l.message&&<p className="text-sm mt-2 line-clamp-2">{l.message}</p>}<p className="text-xs text-muted-foreground mt-2">{new Date(l.created_at).toLocaleString()}</p></div><div className="flex gap-2 flex-wrap"><Select value={l.status} onValueChange={(v:LeadStatus)=>update(l,v)}><SelectTrigger className="w-36"><SelectValue/></SelectTrigger><SelectContent>{statuses.map(s=><SelectItem key={s} value={s} className="capitalize">{s}</SelectItem>)}</SelectContent></Select>{l.customer_id?<Button variant="outline" disabled><UserCheck size={16} className="mr-2"/>Customer linked</Button>:<Button variant="outline" onClick={()=>convert(l)}><UserCheck size={16} className="mr-2"/>Create customer</Button>}</div></div></CardContent></Card>)}</div>}
